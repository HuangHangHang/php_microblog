<?php
    ...
    //回复微博
    if($type == 1){
        $parent_user_id = $db->single('select user_id from tb_post where id = :id',
                                       array('id'=>$pid));//回复id
    }
    $parent_user_id = isset($parent_user_id) ? $parent_user_id : 0 ;

    $addtime  = time();
    $sql      = "insert into tb_post (user_id,username,content,pictures,addtime,pid,post_type,parent_user_id)
    ...
    {
        case 0:
            $db->query('update tb_user set posts_num = posts_num + 1 where id = :user_id',
                        array('user_id'=>$user_id));
      break;
        case 1:
            $db->query('update tb_post set comment_num = comment_num + 1 where id = :pid',
                        array('pid'=>$pid));
            break;
        case 2:
            $db->query('update tb_user set posts_num   = posts_num + 1   where id = :user_id',
                        array('user_id'=>$user_id));
            $db->query('update tb_post set forward_num = forward_num + 1 where id = :pid',
                        array('pid'=>$pid));
      break;
    }
    ...
    if(strstr($content,"@")){  //判断微博内容中是否含有“@”字符
        ...
        if($users_array){
            ...
            for($i = 0;$i < $count; $i++){
                //mr_user表中查找用户id是否存在
                $select_sql = "select id from tb_user where username = '".$users_array[$i]."'";
                $user_id    = $db->single($select_sql,MYSQL_ASSOC);
                if($user_id){
                    //@信息写入到mr_at表
                    $insert_sql = "insert into tb_at (user_id,post_id) values ( :user_id , :post_id)";
