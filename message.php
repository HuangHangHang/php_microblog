<?php
    ...
    $sql = "SELECT * FROM tb_post where parent_user_id = :user_id ORDER BY addtime DESC";
    $total = $db->single("select count(*) from tb_post where parent_user_id = :user_id",array('user_id'=>$user_id));
    ...
    foreach($message as $vo){
        $row = $db->row('select * from tb_post where id = :pid',array('pid'=>$vo['pid']));
