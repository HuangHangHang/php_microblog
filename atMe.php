<?php
    ...
    $total = $db->single("select count(*) from tb_at where user_id = :user_id",array('user_id'=>$user_id));//记录总条数

    if (!empty($_GET['page']) && $total != 0 && $curpage > ceil($total / $showrow)){
        $curpage = ceil($total_rows / $showrow); //当前页数大于最后页数，取最后一页
    }
    $sql = "select * from tb_at where user_id = :user_id order by id desc";
    ...
    if(isset($at_lists)){
        ...
        foreach($at_lists as $vo){
            ...
            $post['avatar']  = $avatar;
            $status          = $db->single('select status from tb_collect where user_id = :uid and post_id = :pid',array('uid'=>$vo['user_id'],'pid'=>$vo['post_id']));
