<?php
    ...
    $total = $db->single("select count(*) from tb_post where  post_type != 1 and user_id = :friend_id",array('friend_id'=>$friend_id));//记录总条数

    if (!empty($_GET['page']) && $total != 0 && $curpage > ceil($total / $showrow)){
        $curpage = ceil($total_rows / $showrow); //当前页数大于最后页数，取最后一页
    }
    //获取数据
    $sql  = "SELECT * FROM tb_post where ( post_type != 1 and user_id = :friend_id) ORDER BY addtime DESC";
    ...
    foreach($row as $vo){
        ...
        $status        = $db->single('select status from tb_collect where user_id = :uid and post_id = :pid',array('uid'=>$_SESSION['user']['id'],'pid'=>$vo['id']));
