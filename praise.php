<?php
    ...
    /** 查看用户是否已经点赞 **/
    $praise    = $db->row('select * from tb_praise where user_id = :user_id and post_id = :post_id',
                           array('user_id'=>$user_id,'post_id'=>$post_id));
    if(!$praise){   //没有点赞
        $sql    = 'insert into tb_praise (user_id,post_id) value (:user_id,:post_id)';
        $insert = $db->query($sql,array("user_id"=>$user_id,"post_id"=>$post_id));
        //微博点赞数加1
        $db->query('update tb_post set praise_num   = praise_num + 1 where id = :post_id',array('post_id'=>$post_id));
        echo 1;
    }else{      //已经点赞
        echo 0;
    }
