<?php
    ...
    /** 判断用户是否已经收藏该微博 **/
    $collect    = $db->row('select * from tb_collect where user_id = :user_id and post_id = :post_id',
                            array('user_id'=>$user_id,'post_id'=>$post_id));
    if(!$collect){   //未收藏，本次首测收藏，写入mr_collect表
        $sql    = 'insert into tb_collect (user_id,post_id,status) value (:user_id,:post_id,1)';
        $insert = $db->query($sql,array("user_id"=>$user_id,"post_id"=>$post_id));
        echo 1;
    }else{
        $sql = 'update tb_collect set status = :status where user_id = :user_id and post_id = :post_id';
        if($collect['status'] == 0){ //之前已取消收藏，再次收藏，写入mr_collect表
            $db->query($sql,array('status'=>1,'user_id'=>$user_id,'post_id'=>$post_id));
            echo 1;
        }else{                      //已经收藏
            $sql = 'update tb_collect set status = 0 where id = :id';
