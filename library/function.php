...
function get_user_avatar($user_id){
    $db     = new Db();
    $sql    = 'select avatar from tb_user where id = '. $user_id;
...
function is_follow($friend_id){
    $user_id = $_SESSION['user']['id'];
    $sql = 'select status from tb_friends where user_id = :user_id and friend_id =:friend_id';
    $db  = new Db();
...
function get_post_info($id){
    $db    = new Db();
    $post  = $db->row('select * from tb_post where id = '.$id);
...
function get_post_number($user_id){
    $db   = new Db();
    $sql  = 'select count(*) from tb_post where post_type != 1 and user_id = :uid';
