<?php
    ...
    $total   = $db->single("select count(*) from tb_collect where status = 1 and  user_id = :user_id ",array('user_id'=>$user_id));//记录总条数

    if (!empty($_GET['page']) && $total != 0 && $curpage > ceil($total / $showrow)){
        $curpage = ceil($total / $showrow); //当前页数大于最后页数，取最后一页
    }
    $sql  = "select * from tb_collect where status = 1 and  user_id = :user_id order by id desc";
