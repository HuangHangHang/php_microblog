<?php
    ...
    $post_info  = $db->row("SELECT * FROM tb_post  where id = :post_id",array('post_id'=>$post_id)); //查找post_id的微博内容
    $avatar     = get_user_avatar($post_info['user_id']); //获取用户头像
    $post_info['avatar'] = $avatar; //头像赋值
    //收藏
    $status    = $db->single('select status from tb_collect where user_id = :user_id and post_id = :post_id',array('user_id'=>$post_info['user_id'],'post_id'=>$post_info['id']));
    ...
    $total = $db->single('select count(*) from tb_post where pid = :post_id and post_type = 2',array('post_id'=>$post_id));
    if (!empty($_GET['page']) && $total != 0 && $curpage > ceil($total / $showrow)) {
        $curpage = ceil($total_rows / $showrow); //当前页数大于最后页数，取最后一页
    }

    //获取所有回复数据
    $sql  = "SELECT * FROM tb_post where pid = :post_id and post_type = 2 ORDER BY addtime DESC";
